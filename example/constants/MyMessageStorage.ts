...
function getAllMessages(callback: (messages: Message[]) => void): void {
...
const MyMessageStorage: CustomMessageStorage = {
  ...
  async find(messageId: string, callback: (message: Message) => void): Promise<void> {
  ...
  findAll(callback: (messages: Message[]) => void): void {
    ...
    getAllMessages((messages: Message[]) => {
