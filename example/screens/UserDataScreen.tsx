...
const UserDataScreen: React.FC = () => {
  ...
  const confirmSaveUserHandler = () => {
    ...
    // Fetch current user from server first, then merge changes on top.
    // The SDK sends null for missing fields, which would clear
    // externalUserId and other identity fields if not preserved.
    mobileMessaging.fetchUser(
      (currentUser: UserData) => {
        const cleaned: Record<string, any> = {};
        for (const [k, v] of Object.entries(currentUser || {})) {
          if (v != null && v !== 'null') cleaned[k] = v;
        }
        const merged: UserData = {...cleaned, ...userData};
