        ...
        stage('Install & Build') {
            steps {
                sh "${loadNvm()} " + '''
                    ...
                    test -f plugin/build/ios/withInfobipWebRTCUI.js || (echo "FAIL: iOS WebRTC plugin JS missing" && exit 1)
                    test -f plugin/build/android/withInfobipWebRTCUI.js || (echo "FAIL: Android WebRTC plugin JS missing" && exit 1)
                ...
                stage('Package Check') {
                    steps {
                        sh "${loadNvm()} " + '''
                            ...
                            grep -q "withInfobipWebRTCUI" /tmp/pack-output.txt || (echo "FAIL: WebRTC plugin files missing from package" && exit 1)
        ...
        stage('Prebuild Verification') {
            steps {
                sh "${loadNvm()} " + '''
                    ...
                    grep -q "voip" ios/InfobipExpoExample/Info.plist || (echo "FAIL: voip UIBackgroundMode missing from Info.plist" && exit 1)
                    grep -q 'WebRTCUIEnabled' ios/Podfile || (echo "FAIL: \$WebRTCUIEnabled global flag missing from Podfile" && exit 1)
                    grep -q 'WEBRTCUI_ENABLED' ios/Podfile || (echo "FAIL: WEBRTCUI_ENABLED compiler flag missing from Podfile post_install" && exit 1)
                    ...
                    grep -q "withWebRTCUI" android/build.gradle || (echo "FAIL: withWebRTCUI flag missing from root build.gradle" && exit 1)
                    grep -q "coreLibraryDesugaringEnabled" android/app/build.gradle || (echo "FAIL: coreLibraryDesugaring not configured in app/build.gradle" && exit 1)
                    grep -q "desugar_jdk_libs" android/app/build.gradle || (echo "FAIL: desugar_jdk_libs dependency missing from app/build.gradle" && exit 1)
                    grep -q "android.permission.CAMERA" android/app/src/main/AndroidManifest.xml || (echo "FAIL: CAMERA permission missing from AndroidManifest.xml" && exit 1)
                    grep -q "android.permission.RECORD_AUDIO" android/app/src/main/AndroidManifest.xml || (echo "FAIL: RECORD_AUDIO permission missing from AndroidManifest.xml" && exit 1)
...
╚════════════════════════════════════════════════╝
                    ...
                    sh "${loadNvm()} " + """
                        GIT_AUTHOR_EMAIL="\${GIT_CREDENTIALS_USR}"
                        case "\${GIT_AUTHOR_EMAIL}" in
                            *@*) ;;
                            *) GIT_AUTHOR_EMAIL="\${GIT_AUTHOR_EMAIL}@infobip.com" ;;
                        esac
                        ...
                        for attempt in {1..12}; do
                            if npm view infobip-mobile-messaging-expo-plugin@${releaseVersion} version; then
                                echo "Published successfully"
                                break
                            fi

                            if [ "\${attempt}" -eq 12 ]; then
                                echo "Package was published but version ${releaseVersion} was not visible after 120 seconds"
                                exit 1
                            fi

                            echo "Package version ${releaseVersion} is not visible yet. Retrying in 10 seconds..."
                            sleep 10
                        done
                    """
